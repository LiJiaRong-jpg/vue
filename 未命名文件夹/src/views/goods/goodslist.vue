<template>
  <div>
    ...
    <div>
      ...
      <el-button type="primary" icon="el-icon-search" @click="load" style="position: relative;top: -15px;" v-permission="'operation:goods:list'">查找
      </el-button>
      <el-button type="primary" icon="el-icon-edit" @click="add" style="position: relative;top: -15px;" v-permission="'operation:goods:create'">添加
      </el-button>

      <el-button :disabled="option.length === 0" style="position: relative;top: -15px;" type="danger"
        icon="el-icon-delete" @click="optionDelete" v-permission="'operation:goods:batchdelete'">批量删除</el-button>
    </div>
    ...
    <el-table :data="tableData.items" size="small" @selection-change="handleSelectionChange" border
      ...
      <el-table-column align="center" label="操作" width="250" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="add(scope.row)" v-permission="'operation:goods:edit'">编辑</el-button>
          <el-button v-if="scope.row.status == 1" type="warning" size="mini" @click="STATUS(scope.row)" v-permission="'operation:goods:edit'">下架</el-button>
          <el-button v-else type="success" size="mini" @click="STATUS(scope.row)" v-permission="'operation:goods:edit'">上架</el-button>
          <el-button type="danger" size="mini" @click="DELETE(scope.row)" v-permission="'operation:goods:delete'">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
...
<script>
  ...
  export default {
    ...
    methods: {
      load() {
        const that = this
        if (that.form.categoryId.length > 0) {
          that.form.categoryId = that.form.categoryId[that.form.categoryId.length - 1]
        }
