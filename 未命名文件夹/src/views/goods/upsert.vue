<template>
  <div>
    ...
    <el-card class="box-card">
      <h3>商品（Spu）介绍</h3>
      <el-form :model="form" label-width="100px" :rules="sku" ref="form" :show-message="true">
        ...
        <el-form-item label="商品图片">
          ...
          <el-upload :action="img" :limit="5" :on-exceed="onExceed" :on-success="onSuccess" :on-remove="onRemove"
            multiple accept=".jpg, .jpeg, .png, .gif" list-type="picture-card">
        ...
        <el-form-item label="所属类目" prop="category">
          <el-cascader :options="categoryList" v-model="form.categoryIds" expand-trigger="hover"
    ...
    <el-card class="box-card">
      ...
      <el-dialog :visible.sync="skuShow" title="添加商品类型（Sku）">
        <el-form ref="skuForm" :model="skuForm" status-icon label-position="left" label-width="100px"
    ...
    <el-card class="box-card">
      ...
      <el-dialog :visible.sync="attributeShow" title="设置商品参数">
        <el-form :model="attributeForm" :data="form.attributeList" label-width="100px"
          style="width: 400px; margin-left:50px;">
    ...
    methods: {
      ...
      addProduct() {
        ...
        that.$refs.form.validate((valid) => {
          if (valid) {
            AddProduct(that.form).then(function(reds){
      ...
      updateProduct(){
          ...
          that.$refs.form.validate((valid) => {
            if (valid) {
              UpdateProduct(that.form).then(function(reds){
                if (reds.data.errmsg=="成功") {
                  that.$router.push({
                    path: '/goods/goodslist'
                  })
