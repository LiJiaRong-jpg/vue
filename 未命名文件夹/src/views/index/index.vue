<template>
    ...
    <el-aside width="auto" style="background-color: #314156;height: 100vh;">
      <el-menu :default-active="active" class="el-menu-vertical-demo" :collapse="isCollapse" unique-opened router>
        ...
        <el-submenu v-for="(item,index) in router" :index="index+''" :key="index">
          <template slot="title" class="el-submenu__title"><i :class="item.i"></i><span>{{item.title}}</span></template>
          <el-menu-item-group style="background-color: #c;">
            <el-menu-item v-for="(Item,Index) in item.children" :key="Index" :index="Item.path" :data-title="Item.title" :data-titl="Item.titl" @click="title">{{Item.titl}}</el-menu-item>
          </el-menu-item-group>
        </el-submenu>
      </el-menu>
    </el-aside>
    ...
    <el-container>
      ...
      <el-header style="text-align: right; font-size: 12px;background-color: white;border-bottom: 1px solid #dcdfe6;">
        ...
        <i class="el-icon-s-fold" style=" float: left;line-height: 60px;font-size: 20px;" @click="iconFont"></i>
            ...
            <i @click="breadcrumb" style="float: left;font-size: 25px;margin-left: 10px;padding-right: 5px;" :class="TITLE!=='首页'?'i':''">首页</i>
        ...
        <el-dropdown trigger="click" style="line-height: 15px;vertical-align: middle;">
      ...
      <div ref="qwer" style="overflow-x: clip;width: 100%;border-bottom: 1px solid #dcdfe6;height: 40px;white-space: nowrap;" @mousewheel="mouse">
          <el-button v-for="(item,index) in arr" :key="index" style="padding: 5px 10px;margin: 5px 10px;" @click="Buttom(index)" @contextmenu.prevent.native="openMenu($event,index)">{{item.titl}}<i class="el-icon-close" @click="Dele(index)"></i></el-button>
      </div>
  ...
  <div class="muen" ref="muen" style="width: 100px;height: 130px;position: fixed;color: black;box-shadow: 2px 2px 3px 0 rgb(0 0 0 / 30%);background-color: white;" v-show="SHOW" @click="closeMenu">
...
export default {
  data () {
    return {
      ...
      TITLE: '首页', // 显示当前所在页面
      arr: [{ titl: '首页', title: '首页', route: '/dashboard' }], // 跳转标签
      SHOW: false, // 右键菜单的显隐
  ...
  methods: {
    ...
    Buttom (e) { // 跳转路由
      ...
      this.$router.push({ path: this.arr[e].route }).catch(err => {
        console.log(err)
      })
    },
    breadcrumb () { // 点击面包屑
    ...
    Dele (e) { // 删除跳转标签
      ...
      if (this.$route.path !== this.arr[e].route) { // 如果当前路由与删除标签的路由不一样
        ...
        if (this.arr.length - 1 === e) { // 如果删除的是最后一个
          ...
          this.$router.push({ path: this.arr[e - 1].route }).catch(err => {
            console.log(err)
          })
        } else {
          this.arr.splice(e, 1)
      ...
      if (this.arr.length < 1) {
        this.arr = [{ titl: '首页', title: '首页', route: '/dashboard' }]
        this.$router.push({ path: '/dashboard' }).catch(err => {
          console.log(err)
        })
        this.TITLE = '首页'
    ...
    Refresh () { // 右键刷新页面
      if (this.$route.path === this.arr[this.muen].route) { // 判断刷新的页面是不是当前页面
        this.$router.push({ path: '/back' }).catch(err => {
          console.log(err)
        // 在back页面的beforeCreate中写一个返回上一级页面
        })
      } else {
        // 直接跳转到该路由
        this.$router.push({ path: this.arr[this.muen].route }).catch(err => {
          console.log(err)
        })
      }
    },
