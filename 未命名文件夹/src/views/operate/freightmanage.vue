  ...
  <div>
    ...
    <el-table :data="tableData.items" style="width: 95%;margin: auto;margin-top: 20px;"
      ...
      <el-table-column align="center" label="操作" prop="type" width="200px;">
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="UPDARE(scope.row)">修改</el-button>
      ...
      <div style="overflow-y: auto;width: 100%;height: 80vh;">
        <el-form ref="addForm" :model="addForm" status-icon label-position="left" label-width="150px"
          ...
          <el-form-item label="发货期限" prop="deliveryDeadline">
            <el-input v-model="addForm.deliveryDeadline" clearable placeholder="">
          ...
          <el-form-item v-if="addForm.freeShipping == 2" label="默认包邮额度" prop="defaultFreePrice">
            <el-input v-model="addForm.defaultFreePrice" placeholder="默认包邮额度">
              <template slot="append">
                元
              </template>
            </el-input>
          </el-form-item>
          <div v-if="addForm.freeShipping!=1">
            <el-form-item label="计费首次数量" prop="defaultFirstNum">
              <el-input v-model="addForm.defaultFirstNum" clearable placeholder="">
                <template slot="append">
                  件
            ...
            <el-form-item label="计费首次价格" prop="defaultFirstPrice">
              <el-input v-model.number="addForm.defaultFirstPrice" clearable placeholder="">
                <template slot="append">
                  元
                </template>
              </el-input>
            ...
            <el-form-item label="计费续件数量" prop="defaultContinueNum">
              <el-input v-model.number="addForm.defaultContinueNum" clearable placeholder="">
                <template slot="append">
                  件
            ...
            <el-form-item label="计费续件价格" prop="defaultContinuePrice">
              <el-input v-model="addForm.defaultContinuePrice" clearable placeholder="">
                <template slot="append">
                  元
      ...
      <el-dialog title="添加指定地区" :visible.sync="Show" :append-to-body="true">
        ...
        <el-form ref="addRess" :model="addRess" status-icon label-position="left" label-width="100px"
          ...
          <el-form-item label="包邮门栏" prop="freePrice">
            <el-input v-model.number="addRess.freePrice" clearable>
              <template slot="append">
                元
              </template>
            </el-input>
          ...
          <el-form-item label="首次数量" prop="firstNum">
            <el-input v-model.number="addRess.firstNum" clearable>
              <template slot="append">
                件
              </template>
            </el-input>
          ...
          <el-form-item label="首次价格" prop="firstMoney">
            <el-input v-model.number="addRess.firstMoney" placeholder="" clearable>
              <template slot="append">
                元
              </template>
            </el-input>
          ...
          <el-form-item label="续件数量" prop="continueNum">
            <el-input v-model.number="addRess.continueNum" placeholder="" clearable>
              <template slot="append">
                件
              </template>
            </el-input>
          ...
          <el-form-item label="续件价格" prop="continueMoney">
            <el-input v-model.number="addRess.continueMoney" placeholder="" clearable>
              <template slot="append">
                元
              </template>
            </el-input>
          ...
          <el-form-item label="指定地区" prop="designatedArea">
            <template>
              <el-checkbox-group v-model="addRess.designatedArea">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      for (var i = 0; i < Row.freightTemplateCarriageDOList.length; i++) {//地址是一个JSON字符串，需转换
        Row.freightTemplateCarriageDOList[i].designatedArea=JSON.parse(Row.freightTemplateCarriageDOList[i].designatedArea)
      }
      // Row.freightTemplateCarriageDOList[0].designatedArea=JSON.parse(Row.freightTemplateCarriageDOList[0].designatedArea)
        this.addForm = {
        ...
        this.addForm.freeShipping = 0
        this.title = '编辑'
        this.show = true
      },
      AddRess() { //添加特殊运费区
        const _ress = this.addRess
        if (_ress.freePrice >= 0 && _ress.continueMoney >= 0 && _ress.designatedArea.length != 0) {
          if (_ress.firstNum > 0 && _ress.continueNum > 0) {
            ...
            this.addForm.freightTemplateCarriageDOList.push(this.addRess)
            this.addRess = {
              freePrice: 0,
              firstNum: 1,
              firstMoney: 0,
      ...
      addTemplate() { //添加运费模板
        ...
        if (_ress.templateName.trim() != '' && _ress.spuLocation.trim() != '' && _ress.deliveryDeadline >= 0) {
          if (_ress.freeShipping == 0) {
            _ress.defaultFreePrice = 0
            this.AddFreightTemplate()
          } else if (_ress.freeShipping == 1) {
            _ress.defaultFreePrice = 0
