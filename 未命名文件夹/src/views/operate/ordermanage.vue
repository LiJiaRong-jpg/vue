      ...
      <el-table :data="tableData.items" size="small" border fit highlight-current-row>
        ...
        <el-table-column align="center" label="操作" width="300" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            ...
            <el-button v-if="scope.row.status === 20" type="primary" size="mini" @click="detailsGoods(scope.row)">发货
            </el-button>
    ...
    methods: {
      ...
      DeliveryOrder(row) { //导出单行订单
        ...
        import('../../components/Excel.js').then(excel => {
          ...
          excel.export_json_to_excel2(header, arr, title, '订单信息')
        })
      },
      Summary() { //汇总
        const that = this
      ...
      detailsGoods(row) { //点击发货
        ...
        this.deliverGoodsForm.orderNo = row.orderNo;
      }
    }
  }
</script>
