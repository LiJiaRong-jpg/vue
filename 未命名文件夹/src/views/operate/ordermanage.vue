<template>
  <div>
    <div>
      ...
      <el-button type="primary" style="position: relative;top: -15px;" icon="el-icon-search" @click="load" v-permission="'operation:order:list'">查找
      </el-button>
      <br />
    ...
    <div style="width: 95%;margin: auto;">
      <el-table :data="tableData.items" size="small" border fit highlight-current-row>
        ...
        <el-table-column align="center" label="操作" width="300" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="details(scope.row)" v-permission="'operation:order:detail'">详情</el-button>
            <el-button v-if="scope.row.status === 20" type="primary" size="mini" @click="DeliveryOrder(scope.row)" v-permission="'operation:order:detail'">配送单</el-button>
            <el-button v-if="scope.row.status === 20" type="primary" size="mini" @click="detailsGoods(scope.row)" v-permission="'operation:order:ship'">发货</el-button>
            <el-button v-if="scope.row.status === 60" type="primary" size="mini" @click="refund(scope.row)" v-permission="'operation:order:refund'">退款</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
