  ...
  <div>
    <div>
      ...
      <el-button style="position: relative;top: -13px;left: 10px;" class="filter-item" type="primary"
        icon="el-icon-search" @click="load"v-permission="'promote:advertisement:query'">查找</el-button>
      <el-button style="position: relative;top: -13px;left: 10px;" class="filter-item" type="primary"
        icon="el-icon-edit" @click="adD" v-permission="'promote:advertisement:create'">添加</el-button>
    </div>
    <div>
      <el-table :data="tableData.items" style="width: 95%;margin: auto;margin-top: 20px;"
        ...
        <el-table-column align="center" label="操作" class-name="small-padding fixed-width">
          <template slot-scope="scope">
            <el-button type="primary" size="mini" @click="updata(scope.row)" v-permission="'promote:advertisement:update'">编辑</el-button>
            <el-button type="danger" size="mini" @click="Delete(scope.row)" v-permission="'promote:advertisement:delete'">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
    ...
    <el-dialog :title="title" :visible.sync="show">
      <el-form ref="form" :model="addForm" label-width="100px">
        ...
        <el-form-item label="广告类型">
          <el-select v-model="addForm.adType" style="width: 200px" class="filter-item" placeholder="请选择广告类型" @change="ADTYPE" >
            <el-option v-for="(key, index) in adType" :key="index" :label="key.name" :value="key.value" />
          </el-select>
        </el-form-item>
...
export default {
  ...
  methods: {
    ...
    ADTYPE (e) {
      if (this.addForm.url != '' && this.addForm.url.indexOf('tid') == -1 && (e == 2 || e == 4)) { // 关联一(不为第三级不为空)且选择分类或首页
        this.mess ? this.mess.close() : ''
        this.mess = this.$message.error('关联一不可为第四级类目或广告类型不选择分类精选与首页轮播下5按钮')
        this.addForm.adType = ''
      }
    },
    handleLink1 (e) { // 关联1
      ...
      let url = ''
      if (e.length != 3 && (this.addForm.adType == 2 || this.addForm.adType == 4)) { // 如果广告类型为分类或首页则只能选择第三级
        this.mess ? this.mess.close() : ''
        this.mess = this.$message.error('请关联第三级类目')
        return
      }
