  ...
  <div>
    ...
    <div class="filter-container" style="margin-top: 20px;margin-left: 20px;margin-bottom: 20px;">
      ...
      <el-button style="position: relative;top: -13px;left: 10px;" class="filter-item" type="primary"
        icon="el-icon-search" @click="load" v-permission="'admin:admin:list'">查找</el-button>
      <el-button style="position: relative;top: -13px;left: 10px;" class="filter-item" type="primary"
        icon="el-icon-edit" @click="handleCreate" v-permission="'admin:admin:create'">添加</el-button>
    </div>

    <el-table :data="tableData.items" style="width: 90%;margin: auto;"
      ...
      <el-table-column prop="roleIds" label="管理员角色" align="center">
        <template slot-scope="scope">
          <el-button type="primary" style="padding: 10px;" plain v-for="item in scope.row.roleIds" :key="item">
            {{buttom[item]}}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column prop="address" label="操作" align="center">
        <!-- :formatter="formatter" -->
        <template slot-scope="scope">
          <el-button type="primary" size="mini" @click="handleUpdate(scope)" v-permission="'admin:admin:update'">
            编辑</el-button>
          <el-button type="danger" size="mini" @click="handleDelete(scope)" v-permission="'admin:admin:delete'">
            删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="title" :visible.sync="addAdmin">
      <el-form ref="form" label-width="80px">
        ...
        <el-form-item label="真实姓名">
          <el-input v-model="adminForm.realname" @keyup.native="inputChange($event)"
